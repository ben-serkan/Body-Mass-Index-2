...
const calculateBMI = () => {
  ...
  if (isNaN(height) || isNaN(weight) || height=='' || weight=='') {
    dialogTitle.innerHTML = "Geçersiz bir değer girdiniz!";
    dialogResult.innerHTML = "";
    dialogResultIcon.innerHTML = '<i class="fas fa-exclamation-circle danger"></i>';
  } else {
      if (result < 18.5) {
        dialogResult.innerHTML = 'Zayıfsınız!';
        dialogResultIcon.innerHTML = '<i class="fas fa-exclamation-circle danger"></i>';
      } else if (result >= 18.5 && result < 25) {
        dialogResult.innerHTML = "Kilonuz normal!";
        dialogResultIcon.innerHTML = '<i class="fas fa-check-circle normal"></i>';
      } else if (result >= 25 && result < 30) {
        dialogResult.innerHTML = "Kilonuz fazla!";
        dialogResultIcon.innerHTML = '<i class="fas fa-exclamation-circle warn"></i>';
      } else if (result >= 30) {
        dialogResult.innerHTML = "Obezsiniz!";
        dialogResultIcon.innerHTML = '<i class="fas fa-exclamation-circle danger"></i>';
      }
      dialogTitle.innerHTML =  `VKİ Sonucunuz: ${Math.round(result)}`;
  }
